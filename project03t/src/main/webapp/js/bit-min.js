function bit(s) {var el;if (s instanceof HTMLElement){el=[s];}else if(s.indexOf("<") == 0){
var t = s.substr(1, s.length - 2);el=[document.createElement(t)];	
}else{el=document.querySelectorAll(s);}
el.click=function(l){for(var i=0;i<this.length;i++){if (this[i].addEventListener) {
		  this[i].addEventListener('click',l);
		} else { // <= IE8
		  this[i].attachEvent('onclick',l);
		}
	  }
	  return this;
	};
